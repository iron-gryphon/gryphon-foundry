...
| **Mirror registry** (`create_mirror_registry`) | **Terraform-generated** offline CA + server cert (**not** ACM) | `mirror.<effective_OCP_domain>` — effective domain is `ocp_base_domain` if set, else `route53_hosted_zone_name` (see `terraform.tfvars.example`) | **`mirror.<domain>` → registry** is created in the effective OCP hosted zone when the mirror registry is enabled and a hosted zone exists. Foundry uses the new private zone when `create_ocp_private_zone` is true; otherwise it uses the existing `route53_hosted_zone_name` zone. |
